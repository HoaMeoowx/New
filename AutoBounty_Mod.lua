-- Giao diện GUI cho nút "Next Player"
...
ScreenGui.Parent = lp:WaitForChild("PlayerGui") -- Thay vì CoreGui
...
NextPlayerButton.Position = UDim2.new(0.5, -100, 0.5, -25)
NextPlayerButton.BackgroundColor3 = Color3.fromRGB(0, 170, 255)
NextPlayerButton.TextColor3 = Color3.fromRGB(255, 255, 255)
NextPlayerButton.Font = Enum.Font.SourceSans
NextPlayerButton.TextSize = 24

-- Logic xử lý khi nhấn nút
NextPlayerButton.MouseButton1Down:Connect(function()
    checktarget()
    print("Đã kiểm tra mục tiêu hoặc tìm mục tiêu mới.")
end)

-- Logic tìm và kiểm tra mục tiêu (đã chỉnh sửa và tích hợp)
local enemy = nil
local fromtime = os.time()
local usedEnemy = {}
...
function findtarget()
    ...
    for _, v in pairs(findvalidlistplayer()) do
        if v and v.Parent and v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
            if lp and lp.Parent and lp.Character and lp.Character:FindFirstChild("HumanoidRootPart") then
                local distance = CalcDistance(v.Character.HumanoidRootPart.CFrame, lp.Character.HumanoidRootPart.CFrame)
...
function checktarget()
    ...
    if (checkNotify("died") or checkNotify("tử trận") or checkNotify("safe") or checkNotify("an toàn")) and
        enemy:DistanceFromCharacter(lp.Character:WaitForChild("Head", 9).Position) <= 15 then
        return findtarget()
    end
    if CheckRaidTarget(enemy) then
        return findtarget()
    end
    if lp.PlayerGui.Main.BottomHUDList.SafeZone.Visible and
        enemy:DistanceFromCharacter(lp.Character:WaitForChild("Head", 9).Position) <= 15 then
        return findtarget()
    end
...
function CheckRaidTarget(target)
    return false
end
